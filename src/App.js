...
export default function App() {
  ...
  const getPoints = (user, allChallenges) => {
    ...
    allChallenges.forEach(ch => {
      ...
      completers.sort((a, b) => b.total !== a.total ? b.total - a.total : a.completedTs - b.completedTs);
      const rank = completers.findIndex(c => c.user === user);
      // If nobody finished, only award 2pts to the leader
      const anyFinished = completers.length > 0;
      const leaderEntry = Object.entries(userTotals).sort((a,b) => b[1]-a[1])[0];
      const isLeader = leaderEntry && leaderEntry[0] === user;
      if (anyFinished) {
        if (rank === 0) points += 5;
        else if (rank === 1) points += 4;
        else if (rank === 2) points += 3;
        else if (rank > 2) points += 2;
      } else if (isLeader && userTotals[user] > 0) {
        points += 2;
      }
      // Doubling bonus: 3pts each time user hits another multiple of goal
      const userTotal = userTotals[user] || 0;
      if (userTotal >= ch.goal) {
        const multiples = Math.floor(userTotal / ch.goal) - 1;
        points += multiples * 3;
      }
    });
